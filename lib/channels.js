...
async function registerChannel(chatId, title, description) {
  ...
  //
  // postingFrequencyHours defaults to 6 (roughly 4 posts/day). Search
  // stays capped at once per calendar day regardless of this value —
  // see lib/pipeline.js — so raising posting frequency doesn't touch
  // the Tavily quota.
  const doc = {
...
/**
 * Saves today's search results so multiple posts across the day can
 * reuse them instead of each triggering a fresh Tavily call.
 */
async function saveSearchCache(chatId, cache, dateStr) {
  const db = getDb();
  await db
    .collection(COLLECTION)
    .doc(String(chatId))
    .set({ searchCache: cache, searchCacheDate: dateStr }, { merge: true });
}

async function deactivateChannel(chatId) {
  const db = getDb();
  // merge:true means this creates the doc if it somehow doesn't exist yet,
  // instead of throwing NOT_FOUND — a channel can be removed before its
  // registration ever succeeded (e.g. a transient LLM/API failure).
  await db
    .collection(COLLECTION)
    .doc(String(chatId))
...
async function markPosted(chatId, postText) {
  ...
  await ref.set({ lastPostedAt: Date.now(), recentPosts }, { merge: true });
}
